...
datatable(sample_n(mh2009, 50)) # for knitting the Rmarkdown
```
...
* having the map! head map, two dimensional data frame, you want to use color to present the value. Here is one dimensinoal dataset.
...
# eval = F can not be run but can be printed as example
# for the echo = F means the code can be run but not printed out
