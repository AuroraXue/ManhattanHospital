...
# Table for most recommend hospital
mostRecommendHospital<-as.data.frame(matrix(NA,nrow =1, ncol=10))
colnames(mostRecommendHospital)<-colnames(new_complications)
for(i in 1:10){
  mostRecommendHospital[1,i]<-names(sort(new_complications[,i])[1])
}


library(fmsb)

# i = names of the hospital
df=as.data.frame(rbind(rep(11,10) , rep(0,10) , new_complications[i,]))
